...
def main() -> None:
    ...
    and print the total amount spent by each customer.
    """
    configure_logging()

    try:
        # Configure Spark
        conf: SparkConf = SparkConf().setMaster("local").setAppName("SpendByCustomer")
    ...
    try:
        ...
        # Collect the results
        results: List[Tuple[int, float]] = total_by_customer.collect()

        # Print the results
        for result in results:
            logging.info(f"Customer ID: {result[0]}, Total Spent: {result[1]:.2f}")
    except Exception as e:
